...
remove_entries(int64_t min_tc,
  ...
  for (i = 0; m.ListSize() > i; ++i) {
    ...
    KaxChapterTimeStart *cts = static_cast<KaxChapterTimeStart *>(atom->FindFirstElt(KaxChapterTimeStart::ClassInfos, false));

    if (NULL != cts)
      entries[i].start = uint64(*cts);

    KaxChapterTimeEnd *cte = static_cast<KaxChapterTimeEnd *>(atom->FindFirstElt(KaxChapterTimeEnd::ClassInfos, false));
