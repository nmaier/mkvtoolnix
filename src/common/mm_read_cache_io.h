...
class MTX_DLL_API mm_read_cache_io_c: public mm_proxy_io_c {
protected:
...
public:
  mm_read_cache_io_c(mm_io_c *p_in, size_t p_cache_size, bool p_delete_in = true);
  virtual ~mm_read_cache_io_c();

  virtual uint64 getFilePointer();
  virtual void setFilePointer(int64 offset, seek_mode mode = seek_beginning);
  virtual int64_t get_size();

  static mm_read_cache_io_c *open(const std::string &file_name, size_t cache_size);
...
typedef counted_ptr<mm_read_cache_io_c> mm_read_cache_io_cptr;
