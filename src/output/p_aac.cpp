...
aac_packetizer_c::get_aac_packet(unsigned long *header,
  ...
  if (verbose && (0 < m_bytes_skipped))
    mxwarn_tid(ti.m_fname, ti.m_id, boost::format(Y("Skipping %1% bytes (no valid AAC header found). This might cause audio/video desynchronisation.\n")) % m_bytes_skipped);
  m_bytes_skipped = 0;
...
aac_packetizer_c::set_headers() {
  if (!hack_engaged(ENGAGE_OLD_AAC_CODECID))
    ...
    if (AAC_PROFILE_MAIN == m_profile)
      ...
      mxerror_tid(ti.m_fname, ti.m_id, boost::format(Y("Unknown AAC MPEG-4 object type %1%.")) % m_profile);
    ...
    if (AAC_PROFILE_MAIN == m_profile)
      ...
      mxerror_tid(ti.m_fname, ti.m_id, boost::format(Y("Unknown AAC MPEG-2 profile %1%.")) % m_profile);
  }
