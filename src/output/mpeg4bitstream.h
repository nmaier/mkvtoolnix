...
BitstreamInit(Bitstream * const bs,
  ...
  uint32_t tmp;
  size_t bitpos;
  uint32_t adjbitstream = (uint32_t)bitstream;
...
BitstreamShowBits(Bitstream * const bs, const uint32_t bits) {
...
BitstreamSkip(Bitstream * const bs, const uint32_t bits) {
  bs->pos += bits;
...
BitstreamPos(const Bitstream * const bs) {
  return ((uint32_t)(8 * ((uint32_t)bs->tail - (uint32_t)bs->start) +
                     bs->pos - bs->initpos));
}
...
BitstreamLength(Bitstream * const bs) {
  uint32_t len = (uint32_t)((uint32_t)bs->tail - (uint32_t)bs->start);
...
static void __inline
BitstreamForward(Bitstream * const bs, const uint32_t bits) {
  bs->pos += bits;
...
static uint32_t __inline
BitstreamGetBits(Bitstream * const bs, const uint32_t n) {
...
static void __inline
BitstreamPutBit(Bitstream * const bs, const uint32_t bit) {
  if (bit)
...
static void __inline
BitstreamPutBits(Bitstream * const bs, const uint32_t value,
                 const uint32_t size) {
...
extern VLC sprite_trajectory_len[];
