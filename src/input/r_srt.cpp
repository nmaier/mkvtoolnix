  ...
  : generic_reader_c(p_ti)
{
  ...
  try {
    m_io = mm_text_io_cptr(new mm_text_io_c(new mm_file_io_c(ti.m_fname)));
    ...
    m_subs  = srt_parser_cptr(new srt_parser_c(m_io.get_object(), ti.m_fname, 0));
  ...
  if (verbose)
    mxinfo_fn(ti.m_fname, Y("Using the SRT subtitle reader.\n"));
