...
vobsub_reader_c::vobsub_reader_c(track_info_c &_ti)
  ...
  try {
    idx_file = new mm_text_io_c(new mm_file_io_c(ti.m_fname));
  } catch (...) {
    throw error_c(Y("vobsub_reader: Cound not open the source file."));
  }

  std::string sub_name = ti.m_fname;
  ...
  if (!idx_file->getline2(line) || !starts_with_case(line, id_string.c_str(), len) || (line.length() < (len + 1)))
    mxerror_fn(ti.m_fname, Y("No version number found.\n"));
  ...
  if (version < 7)
    mxerror_fn(ti.m_fname, Y("Only v7 and newer VobSub files are supported. If you have an older version then use the VSConv utility from "
                             "http://sourceforge.net/projects/guliverkli/ to convert these files to v7 files.\n"));

  parse_headers();
  if (verbose)
    mxinfo_fn(ti.m_fname, boost::format(Y("Using the VobSub subtitle reader (SUB file '%1%').\n")) % sub_name.c_str());
}
...
vobsub_reader_c::create_packetizer(int64_t tid) {
  ...
  ti.m_id               = tid;
  ti.m_language         = tracks[tid]->language;
  ...
  mxinfo_tid(ti.m_fname, tid, boost::format(Y("Using the VobSub subtitle output module (language: %1%).\n")) % track->language);
  ti.m_language = "";
}
  ...
  while (1) {
    ...
    if (starts_with_case(line, "delay:")) {
      ...
      if (!parse_timecode(line, timestamp, true))
        mxerror_fn(ti.m_fname, boost::format(Y("line %1%: The 'delay' timestamp could not be parsed.\n")) % line_no);
      delay = timestamp;
    }

    if ((7 == version) && starts_with_case(line, "timestamp:")) {
      if (NULL == track)
        mxerror_fn(ti.m_fname, Y("The .idx file does not contain an 'id: ...' line to indicate the language.\n"));
      ...
      if ((4 != parts.size()) || (13 > parts[1].length()) || (downcase(parts[2]) != "filepos:")) {
        mxwarn_fn(ti.m_fname, boost::format(Y("Line %1%: The line seems to be a subtitle entry but the format couldn't be recognized. This entry will be skipped.\n")) % line_no);
        continue;
      }
    ...
    switch (buf[3]) {
      ...
      case 0xba:                // Packet start code
        ...
        if ((c & 0xc0) == 0x40)
          ...
          if (!track->mpeg_version_warning_printed) {
            mxwarn_tid(ti.m_fname, track_id,
      ...
      case 0xbd:                // packet
        ...
        if ((c & 0xf0) == 0x20) // System-1 stream timestamp
          ...
          if (dataidx > idx + len) {
            mxwarn_fn(ti.m_fname, boost::format(Y("Invalid header length: %1% (total length: %2%, idx: %3%, dataidx: %4%)\n")) % hdrlen % len % idx % dataidx);
            return deliver();
          }
          ...
          if ((pts_flags & 0xc0) == 0x80) {
            ...
            if (!(((buf[0] & 0xf0) == 0x20) && (buf[0] & 1) && (buf[2] & 1) && (buf[4] & 1))) {
              mxwarn_fn(ti.m_fname, boost::format(Y("PTS error: 0x%|1$02x| %|2$02x|%|3$02x| %|4$02x|%|5$02x|\n")) % buf[0] % buf[1] % buf[2] % buf[3] % buf[4]);
              pts = 0;
            } else
          ...
          if (0 > packet_aid) {
            mxwarn_fn(ti.m_fname, boost::format(Y("Bogus aid %1%\n")) % packet_aid);
            return deliver();
          }
      ...
      default:
        if ((0xc0 <= buf[3]) && (buf[3] < 0xf0)) {
          ...
          mxwarn_fn(ti.m_fname, boost::format(Y("Unknown header 0x%|1$02x|%|2$02x|%|3$02x|%|4$02x|\n")) % buf[0] % buf[1] % buf[2] % buf[3]);
          return deliver();
        }
    }
  }
