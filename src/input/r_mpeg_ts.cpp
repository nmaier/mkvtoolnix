...
#define GET_PID(p) ((((uint16_t)p->pid_msb << 8) | p->pid_lsb) & 0x1FFF)
#define CONTINUITY_COUNTER(p) ((unsigned char)p->continuity_counter & 0x0F)
#define SECTION_LENGTH(p) ((((uint16_t)p->section_length_msb << 8) | p->section_length_lsb) & 0x0FFF)
#define CRC32(p) (((uint32_t)p->crc_3msb << 24) | \
                   ((uint32_t)p->crc_2msb << 16)| \
                   ((uint32_t)p->crc_1msb << 8) | \
                   ((uint32_t)p->crc_lsb))
#define PROGRAM_NUMBER(p) (((uint16_t)p->program_number_msb << 8)| p->program_number_lsb)
#define PCR_PID(p) ((((uint16_t)p->pcr_pid_msb << 8) | p->pcr_pid_lsb) & 0x1FFF)
#define PROGRAM_INFO_LENGTH(p) ((((uint16_t)p->program_info_length_msb << 8 ) | p->program_info_length_lsb) & 0x0FFF)
#define ES_INFO_LENGTH(p) ((((uint16_t)p->es_info_length_msb << 8 ) | p->es_info_length_lsb) & 0x0FFF)
  ...
  , file_done(false)
{
  ...
  //m_file  = new mm_file_io_c("/home/dragon/video/dump.m2v", MODE_CREATE);
}
...
mpeg_ts_reader_c::parse_pat(unsigned char *pat) {
    unsigned int i;
    unsigned long elapsed_CRC, read_CRC;
    unsigned short pat_section_length;
    unsigned short local_program_number;
    unsigned int prog_count = 0;
    mpeg_ts_pat_t *pat_header;
    mpeg_ts_pat_section_t *pat_section;
    ...
    pat_section_length = SECTION_LENGTH(pat_header);
    elapsed_CRC = calculate_crc32(pat, 3 + pat_section_length - 4/*CRC32*/);
    read_CRC = CRC32(((mpeg_ts_crc_t *)(pat + 3 + pat_section_length - 4)));
    ...
    prog_count = (pat_section_length - 5 - 4/*CRC32*/) / 4;

    for (i = 0, pat_section = (mpeg_ts_pat_section_t *)(pat + sizeof(mpeg_ts_pat_t)); i < prog_count; i++, pat_section++)
    {
        local_program_number = PROGRAM_NUMBER(pat_section);
...
mpeg_ts_reader_c::parse_pmt(unsigned char *pmt) {
    ...
    unsigned short pmt_section_length = 0;
    unsigned short es_info_length = 0;
    uint16_t pids_found = 0;
    mpeg_ts_pmt_t *pmt_header;
    mpeg_ts_pmt_descriptor_t *pmt_descriptor;
    mpeg_ts_pmt_pid_info_t *pmt_pid_info;
    ...
    pmt_section_length = SECTION_LENGTH(pmt_header);
    elapsed_CRC = calculate_crc32(pmt, 3 + pmt_section_length - 4/*CRC32*/);
    read_CRC = CRC32(((mpeg_ts_crc_t *)(pmt + 3 + pmt_section_length - 4)));
    ...
    pmt_descriptor = (mpeg_ts_pmt_descriptor_t *)(pmt + sizeof(mpeg_ts_pmt_t));
    program_info_length = PROGRAM_INFO_LENGTH(pmt_header);
    ...
    while (pmt_pid_info < (mpeg_ts_pmt_pid_info_t *)(pmt + 3 + pmt_section_length - 4/*CRC32*/)) {
      ...
      es_info_length = ES_INFO_LENGTH(pmt_pid_info);
      ...
      if (track->type != ES_UNKNOWN) {
        ...
        PMT_found = true;
        char fcc[4];
        fcc[0] = track->fourcc >> 24;
        fcc[1] = track->fourcc >> 16;
        fcc[2] = track->fourcc >> 8;
        fcc[3] = track->fourcc & 0x00FF;
        mxverb(3, boost::format("mpeg_ts:parse_pmt: PID %1% has type: %2%\n") % track->pid % fcc);
      }
    }
...
mpeg_ts_reader_c::parse_packet(int id, unsigned char *buf) {
  ...
  tablePid = (int16_t)GET_PID(hdr);
  //mxverb(3, boost::format("mpeg_ts: PID found: 0x" << std::hex << std::uppercase << tablePid << std::endl;
  ...
  if (id != -1 && tracks[id]->processed == false) {
    matchPid = tracks[id]->pid;
    tidx = id;
  } else {
    for (i = 0; i < tracks.size(); i++) {
      if (tracks[i]->pid == tablePid && tracks[i]->processed == false) {
        matchPid = tracks[i]->pid;
        tidx = i;
        break;
      }
    }
  ...
  if (tablePid == matchPid) {
    //mxverb(3, boost::format("mpeg_ts: match for PID found: " << matchPid << std::endl;
    ...
    if (hdr->payload_unit_start_indicator) {
      if (tracks[tidx]->type == PAT_TYPE || tracks[tidx]->type == PMT_TYPE) {
        ...
        if (pes_data->PTS_DTS > 1) { // 10 and 11 mean PTS is present
          int64_t PTS = 0;
          PTS = ((uint64_t)(pes_data->PTS_4msb & 0x07)) << 29;
    ...
    tracks[tidx]->payload.add(payload, payload_size);
    //mxverb(3, boost::format("mpeg_ts: ---------> Written " << (int)payload_size << " bytes for pid " << tablePid << std::endl;
    ...
    if (tracks[tidx]->data_ready == true) {
      mxverb(3, boost::format("mpeg_ts: Table/PES completed (%1%) for PID %2%\n") % tracks[tidx]->payload.get_size() % tablePid);
...
mpeg_ts_reader_c::create_packetizer(int64_t id) {
  char type = 'v';
  if (tracks[id]->type == ES_AUDIO_TYPE) type = 'a';
  else if (tracks[id]->type == ES_SUBT_TYPE) type = 's';
