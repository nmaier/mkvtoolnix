      ...
      case 'v':                 // video track
        ...
        if (t->codec_id == MKV_V_MSCOMP) {
          if ((t->private_data == NULL) ||
            ...
            if (t->v_width != u) {
              if (verbose)
                mxwarn(PFX "(MS compatibility mode, track " LLU ") Matrosa "
                       "says video width is " LLU ", but the BITMAPINFOHEADER "
                       "says %u.\n", t->tnum, t->v_width, u);
              if (t->v_width == 0)
                t->v_width = u;
            }
            ...
            if (t->v_height != u) {
              if (verbose)
                mxwarn(PFX "(MS compatibility mode, track " LLU ") Matrosa "
                       "video height is " LLU ", but the BITMAPINFOHEADER "
                       "says %u.\n", t->tnum, t->v_height, u);
              if (t->v_height == 0)
                t->v_height = u;
            }
            ...
            if (((uint32_t)t->a_sfreq) != u) {
              if (verbose)
                mxwarn(PFX "(MS compatibility mode for track " LLU
                       ") Matroska says that there are %u samples "
          ...
          if ((EbmlId(*(*cluster)[bgidx]) ==
            ...
            if (NULL == block_track) {
              mxwarn(FMT_FN "A block was found at timestamp " FMT_TIMECODE
                     ...
                     ti.fname.c_str(),
                     ARG_TIMECODE_NS(block_simple->GlobalTimecode()),
                     (unsigned int)block_simple->TrackNum());
              continue;
            }

            if (block_track->v_frate != 0)
            ...
            ref_block = static_cast<KaxReferenceBlock *>
              (block_group->FindFirstElt(KaxReferenceBlock::ClassInfos,
                                         false));
            ...
            blockadd = static_cast<KaxBlockAdditions *>
              (block_group->FindFirstElt(KaxBlockAdditions::ClassInfos,
                                         false));
                ...
                if ((block_track->type == 's') &&
                  ...
                  if (blockadd) {
                    for (i = 0; i < blockadd->ListSize(); i++) {
                      ...
                      blockadd_data =
                        &GetChild<KaxBlockAdditional>(*blockmore);
                      memory_cptr
